...
public class HangmanTest {
    ...
    */
   private static final String SHORT_DICTIONARY = "shortDictionary.txt";
    ...
    */
   private static final int GUESS_LIMIT = 7;
    ...
    * is expected; tests that the number of remaining guesses corresponds
    ...
    *                         by each guess
    */
   private void testWordPatternsAndGuessesCount(final List<Character> guesses,
                                                final List<String>
                                                      expectedPatterns,
                                                final List<Integer>
                                                      expectedCounts) {

      HangmanManager manager = setupBasicManager(SHORT_DICTIONARY,
                                                 GOAL_LENGTH, GUESS_LIMIT);
   ...
   public void hangmanConstructorThrowsErrorWhenAnArgumentIsOutOfRange() {
      ...
      try {
         HangmanManager manager = new Hangman(invalidWordList,
                                              validLength,
      ...
      try {
         HangmanManager manager = new Hangman(validWordLst,
                                              invalidLength,
                                              validWrongGuessLimit);
         fail("IllegalArgumentException should have been thrown but wasn't");
      } catch (Exception e) {
    ...
    * HangmanManager interface, but under the current Hangman
    * implementation it is not possible to reach this state since a similar
   ...
   /**
    * Tests that the devious algorithm produces patterns (and internally word
    * pools) that match what is expected. The dictionary is a small selection
    * of five letter words. It contains {joker smile pears queen spade knife
    * spots}. The following characters are selected 's', 'n', 'k', and 'j'.
    * After 's' is selected, the largest word pool is {joker queen knife}.
    * After 'n' is selected, all word pools are of size 1, and the first
    * {joker} is selected. The first two guesses result in no letters being
    * revealed. After the first two guesses, the wordPool contains only a single
    * goal word and the game progresses as normal.
    */
   @Test
   public void deviousPatternsForShortDictionaryMatchExpected() {
      HangmanManager manager = setupBasicManager(SHORT_DICTIONARY, GOAL_LENGTH,
                                                 GUESS_LIMIT);

      List<Character> guessList = Arrays.asList('s', 'n', 'k', 'j');
      List<String> expectedPatterns = Arrays
            .asList("- - - - -", "- - - - -", "- - k - -", "j - k - -");
      List<Integer> expectedCounts = Arrays.asList(0, 0, 1, 1);

      testWordPatternsAndGuessesCount(guessList, expectedPatterns,
                                      expectedCounts);
   ...
   /**
    * Tests that the devious algorithm produces output consistent with an
    * example given for a school assignment. Due to access restrictions, it
    * is not possible to link to the example.
    */
   @Test
   public void testThatDeviousAlgorithmResultsMatchAssignmentExample() {
      List<String> dict = Arrays.asList("ally", "beta", "cool", "deal", "else",
                                        "flew", "good", "hope", "ibex");

      List<Character> guessList = Arrays.asList('e', 'o', 'g', 'c');
      List<String> expectedPatterns = Arrays
            .asList("- - - -", "- o o -", "- o o -", "c o o -");
      List<Integer> expectedCounts = Arrays.asList(0, 2, 0, 1);

      HangmanManager manager = new Hangman(dict, 4, GUESS_LIMIT);

      for (int i = 0; i < guessList.size(); i++) {
         int count = manager.record(guessList.get(i));
         assertThat(manager.pattern(), equalTo(expectedPatterns.get(i)));
         assertThat(count, equalTo(expectedCounts.get(i)));
      }
